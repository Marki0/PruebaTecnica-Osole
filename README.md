...
- **Assets del mockup (PNG):** van en **`public/nikitos/`** (nombre en **minúsculas**). Esa carpeta **tiene que estar en el repositorio** (`git add public/nikitos`, `commit`, `push`) para que al clonar en otra PC existan las imágenes. El comando `php artisan nikitos:link-assets` solo sirve si en cambio tenés los PNG en **`Nikitos/` en la raíz** del proyecto (enlace hacia `public/nikitos`).
...
├── public/                        # index.php, mix, storage enlace, **nikitos/** (PNG del mockup, minúsculas, versionados en Git)
├── resources/
...
- **Tras clonar no hay imágenes:** el sitio lee **`public/nikitos/`** (minúsculas). Esa carpeta tiene que estar en el repo; si no, copiá los PNG ahí y hacé `git add public/nikitos` + `push`. Luego `php artisan db:seed` si necesitás volver a copiar demos a `storage`. `php artisan storage:link` para el panel.
